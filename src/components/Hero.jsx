...
const parent = {
  ...
  visible:{opacity:1, y:0, transition:{staggerChildren: 0.4, duration:0.4}}
}
...
const Hero = () => {
  ...
  return (
    <div id='hero' className='bg-[url("https://plus.unsplash.com/premium_photo-1754251306733-120ef8883f91?q=80&w=2070&auto=format&fit=crop&ixlib=rb-4.1.0&ixid=M3wxMjA3fDB8MHxwaG90by1wYWdlfHx8fGVufDB8fHx8fA%3D%3D")] relative min-h-screen flex flex-col justify-start  text-center bg-cover bg-center bg-fixed bg-no-repeat overflow-hidden' >
      ...
      <motion.div variants={parent} initial="hidden" whileInView="visible" viewport={{once:true, amount:0.3}}  className='relative z-10 text-white text-left space-y-10 px-7 py-10 mt-70 sm:mt-75 md:mt-80'>
        ...
        <motion.div variants={children}  className='mt-15 w-fit mx-auto sm:mx-0 hover:scale-105 transition-all ease-in-out duration-300'>
          <HashLink to='/#form' className="border-2 relative z-10 mt-10 text-white px-5  py-4 rounded-md text-xl sm:text-2xl md:text-3xl">Get Free Appointment</HashLink>
        </motion.div>
      </motion.div>
